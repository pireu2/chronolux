...
\begin{figure*}[t]
\centerline{\includegraphics[width=0.7\textwidth]{architecture.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.35\textwidth]{pipeline.png}}
...
\begin{figure}[htbp]
\begin{algorithmic}[1]
\REQUIRE $PositionMap$, $NormalMap$, $AccumulationBuffer$, $DeltaTime$
\FOR{\textbf{each} thread $id \in \text{DispatchThreadID}$}
    \STATE $rayOrigin \leftarrow PositionMap[id.xy].xyz$
    \STATE $rayNormal \leftarrow NormalMap[id.xy].xyz$
    \IF{$||rayNormal|| < 0.1$}
        \STATE \textbf{return}
    \ENDIF
    \STATE $ray.Origin \leftarrow rayOrigin + rayNormal \times 0.001$
    \STATE $ray.TMin \leftarrow 0.01$
    \STATE $ray.TMax \leftarrow 1000.0$
    \STATE $irradiance \leftarrow \textsc{TraceRadiance}(ray, rayNormal)$
    \STATE $AccumulationBuffer[id.xy] \leftarrow AccumulationBuffer[id.xy] + irradiance \times DeltaTime$
\ENDFOR
\end{algorithmic}
...
\end{equation}
Furthermore, these virtual Lux probes calculate real time Monte Carlo convergence errors compared to theoretical values. Considering the fact that any attempt of GPU to CPU readout introduces serious synchronization issues, the telemetry process proceeds asynchronously utilizing a background headless thread in C\#. The thread safely sums up every hourly delta value as well as maximum exposure and variance metrics, exporting the results to a standard CSV array without interrupting 60 FPS render cycle.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.9\columnwidth]{fig1b_cumulative_avg_dose.png}}
...
\subsection{Direct Beam vs. Diffuse Analysis (NEE Validation)}
Changing the location of the asset in Scenarios C and D is necessary to explore the effects of geometric occlusion and represents the strict mathematical stress test for the NEE functionality.

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.9\columnwidth]{fig5a_beam_vs_diffuse.png}}
\caption{Decomposition of light dosimetry in Scenario A shows effective separation of direct solar beam from diffuse Perez ambient skylight using our Next Event Estimation algorithm, with direct beam dominating the exposure.}
\label{fig:beam_diffuse_a}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.9\columnwidth]{fig5b_beam_vs_diffuse_scenario_d.png}}
\caption{Decomposition of light dosimetry in Scenario D reveals a significantly larger diffuse/indirect composition compared to Scenario A, validating the impact of geometric occlusion.}
\label{fig:beam_diffuse_d}
\end{figure}

As shown in Figure~\ref{fig:beam_diffuse_a} and Figure~\ref{fig:beam_diffuse_d}, our engine is able to distinguish between direct beam irradiance and diffuse ambient skylight. This confirms that our geometric occlusion successfully filters out the direct beam stress and leaves the asset exposed predominantly to Perez diffuse skylight.
